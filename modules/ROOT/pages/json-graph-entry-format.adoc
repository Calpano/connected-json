...
:cj-document: xref:spec-cj.adoc#document[CJ Document]
:cj-graph: xref:spec-cj.adoc#graph[CJ Graph]
:cj-node: xref:spec-cj.adoc#node[CJ Node]
:cj-edge: xref:spec-cj.adoc#edge[CJ Edge]
:cj-endpoint: xref:spec-cj.adoc#endpoint[CJ Endpoint]
:cj-port: xref:spec-cj.adoc#port[CJ Port]
:cj-id: xref:spec-cj.adoc#id[CJ ID]
:cj-label: xref:spec-cj.adoc#label[CJ Label]
...
[[id]]
=== ID
In all places where a {cj-id} can be used, {gef} also allows to state the ID using a JSON `number`.
