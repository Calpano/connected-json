...
:gef: Graph Entry Format (GEF)
:cj: Connected JSON (CJ)

How to interpret various JSON constructs as a graph, mapped to
...
Many formats in the wild are already quite close to {cj} anyway.
{gef} is a superset of https://github.com/jsongraph/json-graph-specification[JSON Graph Format].
...
=== Example
First of all, all {cj} files are valid {gef}.
...
[source,json]
----
include::example$example-1.gef.json5[]
----

=== Change Log
2025-07-14: Version 5.0.0::
* Split spec into two parts: {cj} for writing strict files, where there is always only one option to encode a structure and {gef} which is much more liberal and flexible in parsing.
* Moved `edgeDefault` to {gef}.


== Overview
{gef} uses the same xref:spec-cj.adoc#conceptual_model[conceptual model] as {cj}, but it maps a broader range of JSON structures to the graph model.
...
=== Polymorphism
{gef} uses a kind of 'JSON polymorphism': A property allows multiple JSON types to be used. Interpretation of that JSON value depends on the used type.

- For example, the property `label` is in {gef} either a JSON string or a JSON object (multilingual like in {cj}). Interpretation is defined per property.
...
[[document]]
=== Document
For the default document structure, see
xref:spec-cj.adoc#document[CJ document].

.Root Object as Graph Object
In {gef}, a graph can be stated at the root level, without using a `graph` property.
...
|baseUri|`string`(URI)| Optional. Document-level. Documented in xref:spec-cj.adoc#document[CJ document].
...
|edges| `array`(<<edge>> `[]`) | Optional. Graph-level. Documented in <<graph>>.

|graph| `object`(<<graph>>)
.4+| Optional. Document-level. Can also be used inside graphs. Documented in xref:spec-cj.adoc#document[CJ document] and <<graph>>.
...
|id| `string`
.2+| Optional. Graph-level. Documented in <<graph>>. A number is converted to string to be used as ID.
|id| `number` (integer)

|label| `object` | Optional. Graph-level. Documented in <<graph>>.
|label| `string` | Optional. Graph-level. See <<label>>.

|connectedJson| `object`(xref:spec-cj.adoc#meta-document[Document Metadata]]) | Optional. xref:spec-cj.adoc#meta-document[Graph Metadata].

|nodes| `array`(<<node>> `[]`) | Optional. Graph-level. Documented in <<graph>>.
...
[[id]]
=== ID
In all places where a xref:spec-cj.adoc#id[CJ ID] can be used, {gef} also allows to state the ID using a JSON `number`.
This number is converted at parse-time into the corresponding string.
E.g., the number `3` is converted to the string id `"3"`.
...
In other words: A xref:spec-cj.adoc#label[CJ Label] without language.

[[graph]]
=== Graph
For the default graph structure, see xref:spec-cj.adoc#graph[Graph] in {cj}.
{gef} adds some aliases.
...
This is similar to https://graphviz.org/Gallery/directed/cluster.html[DOT syntax] using `cluster` as a prefix for subgraphs. Having a cluster prefix is like `"compoundNode": true`.
...
|id| `string` | -> xref:spec-cj.adoc#node[CJ Node].`id`: a xref:spec-cj.adoc#id[CJ ID]
|id|`number` (integer) | Is converted to string and interpreted as a xref:spec-cj.adoc#id[CJ ID].

|label| `object` | -> xref:spec-cj.adoc#node[CJ Node].`label`: A xref:spec-cj.adoc#label[CJ Label].
|label| `string` | Optional. See <<label>>.


|meta| `object`(xref:spec-cj.adoc#meta-document[Graph Metadata]]) | Optional. xref:spec-cj.adoc#meta-document[Graph Metadata]].


// Streaming Data
|nodes| `array`(<<node>> `[]`) | 0 to n nodes. Default: Empty.

|edges| `array`(<<edge>> `[]`) | -> xref:spec-cj.adoc#graph[CJ Graph].`edges`: An array of xref:spec-cj.adoc#edge[CJ Edge]
...
|graphs| `array`(<<graph>> `[]`) | -> xref:spec-cj.adoc#graph[CJ Graph].`graphs`: An array of xref:spec-cj.adoc#graph[CJ Graph]s.
...
|label|`string`| Interpreted as `{ "value": x }` with _x_ being the string value. In other words: A xref:spec-cj.adoc#label[CJ Label] without language.

| ports| `array`(<<port>> `[]`) | -> xref:spec-cj.adoc#node[CJ Node].`ports`: an array of xref:spec-cj.adoc#port[CJ Port]

|graphs| `array` (<<Graph>>[]) | -> xref:spec-cj.adoc#node[CJ Node] `graphs`: An array of xref:spec-cj.adoc#graph[CJ Graph]s.
|graph| `object`(<<graph>>) | `graph` allowed as alias for `graphs`
|graph| `array`(<<graph>> `[]`) | `graph` allowed as alias for `graphs`
|graphs| `object`(<<graph>>) | Polymorph object and array
...
|id| `string` | -> xref:spec-cj.adoc#port[CJ Port].`id`: a xref:spec-cj.adoc#id[CJ ID]
|id| `number` | See <<id>>

|label| `object` | -> xref:spec-cj.adoc#port[CJ Port].`label`: A xref:spec-cj.adoc#label[CJ Label].
|label| `string` | See <<label>>

|ports| `array`(<<port>> `[]`) | -> xref:spec-cj.adoc#port[CJ Port].`ports`: An array of xref:spec-cj.adoc#port[CJ Port]s.
|===
...
|id| `string` | -> xref:spec-cj.adoc#edge[CJ Edge].`id`: a xref:spec-cj.adoc#id[CJ ID]
|id| `number` | See <<id>>
...
|label| `object` | -> xref:spec-cj.adoc#edge[CJ Edge].`label`: A xref:spec-cj.adoc#label[CJ Label].
|label| `string` | See <<label>>

|type|`string`|-> xref:spec-cj.adoc#edge[CJ Edge].`type`
|typeUri|`string`|-> xref:spec-cj.adoc#edge[CJ Edge].`typeUri`
|typeNode|`string`| -> xref:spec-cj.adoc#edge[CJ Edge].`typeNode`
|typeNode|`number` (integer)|Interpreted as the <<id>> of the <<node>> that defines the type of edge. Like xref:spec-cj.adoc#edge[CJ Edge].`typeNode`.
...
|graph | `array` (<<graph>> `[]`) | `graph` allowed as alias for `graphs`
|graphs| `object`(<<graph>>) | Polymorph object and array
|graphs| `array` (<<graph>> `[]`) | -> xref:spec-cj.adoc#edge[CJ Edge] `graphs`: An array of xref:spec-cj.adoc#graph[CJ Graph]s.
...
|direction|`string`| -> xref:spec-cj.adoc#endpoint[CJ Endpoint].`direction`. See also <<edge-direction>>.

|node| `string` | -> xref:spec-cj.adoc#endpoint[CJ Endpoint].`node`: A xref:spec-cj.adoc#id[CJ ID]
...
|port| `string` | -> xref:spec-cj.adoc#endpoint[CJ Endpoint].`port`
|port| `number` (integer) | Optional. Interpreted as a xref:spec-cj.adoc#id[CJ ID] to refer to a <<port>>.

|type| `string`| -> xref:spec-cj.adoc#endpoint[CJ Endpoint].`type`
|typeUri| `string`(URI) | -> xref:spec-cj.adoc#endpoint[CJ Endpoint].`typeUri`
|typeNode| `string`| -> xref:spec-cj.adoc#endpoint[CJ Endpoint].`typeNode`
|typeNode|`number` (integer) | Auto-converted to a string and interpreted like xref:spec-cj.adoc#endpoint[CJ Endpoint].`typeNode`.
...
In {gef}, data handling is much more relaxed:
...
How to interpret a hyperedge with, e.g. 4 endpoints as _directed_? In {gef}, the first endpoint is interpreted as `in` and all others as `out`.
