...
.Streaming
For streaming processing, the property order MUST be: (`id`, `label`, `type`, `type-uri`, `type-node`, `directed`), (`source`, `target`, `endpoints`), `graph`. The order within each brace group is not relevant.
...
| target | <<edge>>
| type | <<edge>>, <<endpoint>>
| type-node | <<edge>>, <<endpoint>>
| type-uri | <<edge>>, <<endpoint>>
|===
