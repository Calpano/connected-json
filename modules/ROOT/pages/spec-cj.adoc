...
See also: link:spec-ecj.adoc[Extended Connected JSON].
...
For ways how to interpret similar, much more flexible formats unambiguously as {cj}, look into link:spec-ecj.adoc[Extended CJ].
...
link:spec-ecj.adoc[{ecj}] (ECJ):: A relaxed superset of CJ for reading. It offers many aliases, shortcuts and variants to interpret JSON as as graph. See link:spec-ecj.adoc[Extended CJ Specification].
...
4+| Defined in link:spec-ecj.adoc[Extended CJ]
...
[#streaming]
=== Streaming
JSON in general is not ideal for streaming data, see also link:notes/streaming-json.adoc[Notes on Streaming JSON].
