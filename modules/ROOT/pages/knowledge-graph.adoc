...
If an `object` is used, it is for xref:spec.adoc#multilingual-labels[multilingual text]. For binary content, use `content` as a `string` with BASE64 encoded value.
