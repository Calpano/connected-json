...
As this format is meant for relaxed parsing of various CJ-like JSON dialects, it does not define rules for stream processing. See link:notes/streaming-json.adoc[] and especially link:spec-cj.adoc#streaming[Streaming in CJ] for details.

=== Example
First of all, all {cj} files are valid {ecj}.
So look at link:spec-cj.adoc#example[the example in the CJ specification].
...
== Overview
{ecj} uses the same link:spec-cj.adoc#conceptual_model[conceptual model] as {cj}, but it maps a broader range of JSON structures to the graph model.
...
|connectedJson| `object` | Optional. Document-level. Documented in link:spec-cj.adoc#meta-document[CJ Document Metadata].
...
|meta| `object`(link:spec-cj.adoc#meta-graph[Graph Metadata]]) | Optional. link:spec-cj.adoc#meta-graph[Graph Metadata]].
...
[[graph]]
=== Graph
For the default graph structure, see link:spec-cj.adoc#graph[Graph] in {cj}.
...
[[node]]
=== Node
For the default node structure, see link:spec-cj.adoc#node[Node] in {cj}.
...
[[port]]
=== Port
For the default port structure, see link:spec-cj.adoc#port[Port] in {cj}.
...
[[edge]]
=== Edge
For the default edge structure, see link:spec-cj.adoc#edge[Edge] in {cj}.
...
[[endpoint]]
=== Edge Endpoint
For the default endpoint structure, see link:spec-cj.adoc#endpoint[Endpoint] in {cj}.
